...
func CreateTrainingRecord(c echo.Context) error {
	...
	if !student.IsOnFacilityRoster(request.Facility) {
		return echo.NewHTTPError(http.StatusBadRequest,
	...
	if !instructor.IsOnFacilityRoster(request.Facility) {
		return echo.NewHTTPError(http.StatusBadRequest,
	...
	record.Save()
	return c.JSON(http.StatusOK, model.MakeTrainingRecordResponse(&record))
}
...
func GetControllerTrainingRecords(c echo.Context) error {
	...
	var output []*model.TrainingRecord
	for _, v := range records {
		output = append(output, model.MakeTrainingRecordResponse(&v))
	}
